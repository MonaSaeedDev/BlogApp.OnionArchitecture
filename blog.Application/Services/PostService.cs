...
public class PostService(BlogDbContext context) : IPostService
{
    public async Task<int> CreatePostAsync(CreatePostDto createPostDto, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new ArgumentException("Content cannot be null, empty, or whitespace.", nameof(createPostDto.Content));
        }
        ...
        if (!userExists)
        {
            throw new ArgumentException($"Author with ID {createPostDto.UserId} was not found.");
        }
        var post = new Post
        {
